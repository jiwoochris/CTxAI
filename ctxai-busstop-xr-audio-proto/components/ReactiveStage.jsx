...
function RealForest() {
  ...
  const trees = useMemo(() => {
    ...
    // 건너편 둘째 줄 (z -31 ~ -37) — 첫 줄 사이사이를 메워 숲이 두 겹으로 보이게. 카페 앞은 역시 비움
    for (let x = -42; x <= 42; x += 5.1) {
      if (x > -26 && x < -11) continue;
      const j = ((x * 3) % 4 + 4) % 4;
      put(x + 2.1 + (j - 1.5) * 0.8, -31.5 - j * 1.9, 5.6 + (j % 2) * 0.9, j % 2 === 0 ? "fir" : "pine");
    }
