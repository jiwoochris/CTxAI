...
- `/film` (v3·반응형 실시간 영화, 2026-09-10 신규) — 판정 1회가 아니라 체험 내내 연출 상태가 장면 전체를 움직이는 버전. 설계는 `Bus/규격/반응형_실시간_영화.md`
...
- 로컬 실행 시 셸에 `NODE_ENV`·`TURBOPACK`이 잡혀 있으면 CSS 로드·빌드가 깨진다: `env -u NODE_ENV -u TURBOPACK npm run dev`
